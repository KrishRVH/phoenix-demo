...
- `mise run setup`: prepare the database and assets for local development.
- `mise run dev`: run the Phoenix server.
- `mise run fmt`: format.
- `mise run fmt:check`: verify formatting.
- `mise run lint`: lint/static analysis.
- `mise run test`: tests.
- `mise run check`: standard local gate.
- `mise run ci`: extended local gate.

Do not call package managers, compilers, or test runners directly unless you are
fixing the mise task itself.
...
- language outputs: `_build/`, `deps/`, `dist-newstyle/`, `.stack-work/`
- tool caches: `.elixir_ls/`, `cover/`, `doc/`
